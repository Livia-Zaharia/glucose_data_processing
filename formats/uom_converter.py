...
class UoMConverter(CSVFormatConverter):
    ...
    def can_handle(self, headers: List[str]) -> bool:
        ...
        if all(header in clean_headers for header in uom_glucose_headers):
            return True
        elif all(header in clean_headers for header in uom_basal_headers):
            return True
        elif all(header in clean_headers for header in uom_bolus_headers):
            return True
        elif all(header in clean_headers for header in uom_nutrition_headers):
            return True
    ...
    def set_context(self, file_path: Path) -> None:
        """
        Set the context for conversion based on the file path.
